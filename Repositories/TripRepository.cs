...
namespace TransportationManagement.Repositories
{
    public class TripRepository : ITripRepository
    {
        ...
        public List<Trip> GetAllTrips() =>
            _context.Trips.Include(t => t.Vehicle).Include(t => t.Driver).ToList();

        public Trip? GetTripById(int tripId) =>
            _context.Trips.Include(t => t.Vehicle).Include(t => t.Driver).FirstOrDefault(t => t.tripId == tripId);

        public List<Trip> GetTripsByDriverId(int driverId) =>
            _context.Trips.Include(t => t.Vehicle).Include(t => t.Driver).Where(t => t.driverId == driverId).ToList();
