...
namespace TransportationManagement.Repositories
{
    public class FuelRepository : IFuelRepository
    {
        ...
        public List<FuelEntry> GetAllFuelEntries() =>
            _context.FuelEntries.Include(f => f.Vehicle).ToList();

        public FuelEntry? GetFuelEntryById(int fuelId) =>
            _context.FuelEntries.Include(f => f.Vehicle).FirstOrDefault(f => f.fuelId == fuelId);

        public List<FuelEntry> GetFuelEntriesByVehicleId(int vehicleId) =>
            _context.FuelEntries.Include(f => f.Vehicle).Where(f => f.vehicleId == vehicleId).ToList();
